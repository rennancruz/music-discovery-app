    ...
    itunesData.results.forEach((result) => {
      ...
      if (!uniqueResults.has(uniqueKey)) {
        ...
        songElement.innerHTML = `
          <div class="text-left">
              <p class="text-lg text-gray-800">${result.trackName}</p>
              <p class="text-md text-gray-800">${result.artistName}</p>
              </br>
              <p class="text-sm text-gray-800">${result.collectionName}</p>
          </div>
...
function addToPlaylist(song) {
  ...
  if (!songExists) {
    ...
    modalContainer.innerHTML = `
      <div class="bg-white p-4 rounded shadow-lg">
        ...
        <div class="flex justify-between items-center">
          ...
          <button class="text-gray-500 hover:text-gray-800" onclick="closeModal()">:heavy_multiplication_x:</button>
        </div>
        <!-- Modal Content -->
    ...
    alert(
      `${song.trackName} by ${song.artistName} is already in your playlist.`
    );
  }
}
